...
# adjust eta such that v * gamma = || 0.00700175 = v * 6*np.pi*eta*Rp average radius
...
lambd = (2*np.pi*5.6e-5)/39.107
p = 1040 
alpha = 7.02158e-14
tao = 0.00048528
kbt = 1.38064852e-23 * 300 
vavg = np.sqrt((4*kbt) / (3*lambd*alpha))
vavgTilda = tao*vavg/lambd #1040kg/m^3
j = 0
TC = []
eta = 0.00700175 / vavgTilda 
...
for i in range(1):
    ...
    while Results.count()[0] != N:
        ...
        gamma = (6*np.pi*eta*Rp*tao)/M #uses specific mass with average eta 
        dRhodT = vx*np.sin(theta)*np.cos(phi)+vy*np.sin(theta)*np.sin(phi)+vz*np.cos(theta)
        if dRhodT <= 0:
            Results = pd.concat([pd.DataFrame([[vx,vy,vz,rho,theta,phi,dRhodT,0,0,Rp,0,None]],columns = Results.columns),Results],ignore_index = True)
        print(Results)
            #looking for velocities towards the resonator 
############################################################  Function #######################################################################################
